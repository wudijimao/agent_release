...
const riskLabelMap: Record<SkillRiskLevel, string> = { low: '低风险', medium: '中风险' };
const riskClassMap: Record<SkillRiskLevel, string> = { low: 'bg-skillRiskLow text-primary', medium: 'bg-skillRiskMedium text-warning' };

export function SkillPage({ isSidebarOpen, skills, onOpenSidebar, onCreateSkill, onInstall, onUninstall }: SkillPageProps) {
  ...
  return (
    <div className="relative flex h-full w-full flex-col bg-surface">
      ...
      <div className={`flex-1 overflow-y-auto px-4 pt-4 md:px-8 md:pt-6 lg:px-10 ${isBatchMode ? 'pb-32' : 'pb-12'}`}>
                ...
                {filteredSkills.map((skill) => {
                  ...
                  return (
                    <article key={skill.id} className={`rounded-lg border p-4 transition-shadow hover:shadow-sm ${cardClass}`} onMouseEnter={() => setHoveredSkillId(skill.id)} onMouseLeave={() => setHoveredSkillId((current) => current === skill.id ? null : current)}>
                      ...
                      <div className="mt-4 flex items-center justify-between gap-3"><div className="flex flex-wrap gap-2">{skill.tags.map((tag) => <span key={`${skill.id}-${tag}`} className="inline-flex items-center rounded-full bg-skillTagSurface px-3 py-1.5 text-xs text-tertiaryText">{tag}</span>)}</div>{!isBatchMode && <button type="button" onClick={() => runSingleAction(skill)} className={`shrink-0 items-center rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${hoveredSkillId === skill.id ? 'inline-flex' : 'hidden'} ${skill.installed ? 'bg-skillTagSurface text-primaryText' : 'bg-primary text-white'}`}>{skill.installed ? '卸载' : '安装'}</button>}</div>
                    </article>
                  );
                })}
              </div>
            ) : <div className="mt-4 flex h-36 items-center justify-center rounded-lg border border-lineSubtle text-sm text-tertiaryText">暂无匹配的 Skills</div>}
          </section>
        </div>
      </div>

      {isBatchMode && <div className="pointer-events-none absolute inset-x-0 bottom-0 z-30 border-t border-lineSubtle bg-surface"><div className="pointer-events-auto mx-auto flex max-w-[1240px] items-center justify-between px-4 py-3 md:px-8 lg:px-10"><span className="text-sm text-secondaryText">已选择 {selectedSkillIds.length} 条 Skill</span><div className="flex items-center gap-2"><button type="button" onClick={toggleBatchMode} className="rounded-md border border-lineSubtle bg-surface px-3 py-1 text-sm text-secondaryText transition-colors hover:bg-skillCancelHover">取消</button><button type="button" onClick={runBatchAction} disabled={!selectedSkillIds.length} className="rounded-md bg-primary px-3 py-1 text-sm font-medium text-white transition-colors hover:bg-primary-hover disabled:cursor-not-allowed disabled:bg-skillActionDisabled">{activeTab === 'installed' ? '批量卸载' : '批量安装'}</button></div></div></div>}
    </div>
  );
}
